...
CREATE TABLE `landlord` (
  `landlord_id` int NOT NULL AUTO_INCREMENT COMMENT '房东ID，主键，自动增长',
  `name` varchar(100) NOT NULL COMMENT '房东姓名，非空',
  `contact_number` varchar(15) NOT NULL COMMENT '联系电话，非空',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='房东信息表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `leaseagreement` (
  `agreement_id` int NOT NULL AUTO_INCREMENT COMMENT '合同ID，主键，自动增长',
  `tenant_id` int DEFAULT NULL COMMENT '租户ID，外键引用租户表',
  `property_id` int DEFAULT NULL COMMENT '房源ID，外键引用房源表',
  `start_date` date NOT NULL COMMENT '合同开始日期，非空',
  `end_date` date NOT NULL COMMENT '合同结束日期，非空',
  `monthly_rent` decimal(10,2) NOT NULL COMMENT '每月租金，非空',
  `agreement_type` varchar(50) DEFAULT NULL COMMENT '合同押金支付方式',
  `status` enum('Active','Inactive','Expired') DEFAULT 'Active' COMMENT '合同状态，默认为有效',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，默认为当前时间戳',
...
CREATE TABLE `property` (
  `property_id` int NOT NULL AUTO_INCREMENT COMMENT '房源ID，主键，自动增长',
  `landlord_id` int DEFAULT NULL COMMENT '房东ID，外键引用房东表',
  `address` varchar(255) NOT NULL COMMENT '地址，非空',
  `rent_price` decimal(10,2) NOT NULL COMMENT '租金价格，非空',
  `deposit` decimal(10,2) DEFAULT NULL COMMENT '押金',
  `available` tinyint(1) DEFAULT '1' COMMENT '是否可用，默认为可用',
...
CREATE TABLE `propertyattributes` (
  `attribute_id` int NOT NULL AUTO_INCREMENT COMMENT '属性ID，主键，自动增长',
  `property_id` int NOT NULL COMMENT '房源ID，外键引用房源表',
  ...
  `number_of_beds` int DEFAULT NULL COMMENT '几人间（几张床）',
  `room_structure` varchar(50) DEFAULT NULL COMMENT '房间结构（几室几厅）',
...
CREATE TABLE `renewal` (
  `renewal_id` int NOT NULL AUTO_INCREMENT COMMENT '续租ID，主键，自动增长',
  `agreement_id` int DEFAULT NULL COMMENT '合同ID，外键引用租赁合同表',
  `new_start_date` date NOT NULL COMMENT '新的开始日期，非空',
  `new_end_date` date NOT NULL COMMENT '新的结束日期，非空',
  `renewed_monthly_rent` decimal(10,2) NOT NULL COMMENT '续租后每月租金，非空',
  `renewed_status` enum('Pending','Approved','Rejected') DEFAULT 'Pending' COMMENT '续租状态，默认为待定',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，默认为当前时间戳',
...
INSERT INTO `sys_logininfor` VALUES (1,'admin','127.0.0.1','内网IP','Chrome 12','Windows 10','0','登录成功','2024-09-28 20:19:35'),(2,'admin','127.0.0.1','内网IP','Chrome 12','Windows 10','0','登录成功','2024-09-29 14:03:48'),(3,'admin','127.0.0.1','内网IP','Chrome 12','Windows 10','0','登录成功','2024-09-29 16:36:07');
...
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `tenant` (
  `tenant_id` int NOT NULL AUTO_INCREMENT COMMENT '租户ID，主键，自动增长',
  `name` varchar(100) NOT NULL COMMENT '租户姓名，非空',
  `contact_number` varchar(15) NOT NULL COMMENT '联系电话，非空',
  `email` varchar(100) DEFAULT NULL COMMENT '邮箱地址',
  `identification_number` varchar(20) DEFAULT NULL COMMENT '身份证号',
  `viewing_demand` tinyint(1) DEFAULT '0' COMMENT '看房需求，默认为不需要看房',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，默认为当前时间戳',
...
CREATE TABLE `usermanagement` (
  `user_id` int NOT NULL AUTO_INCREMENT COMMENT '用户ID，主键，自动增长',
  ...
  `email` varchar(100) DEFAULT NULL COMMENT '邮箱地址',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，默认为当前时间戳',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间，默认为当前时间戳',
  PRIMARY KEY (`user_id`)
...
CREATE TABLE `viewing` (
  `viewing_id` int NOT NULL AUTO_INCREMENT COMMENT '看房ID，主键，自动增长',
  `tenant_id` int DEFAULT NULL COMMENT '租户ID，外键引用租户表',
  `property_id` int DEFAULT NULL COMMENT '房源ID，外键引用房源表',
  `scheduled_time` datetime NOT NULL COMMENT '预约看房时间，非空',
  `status` enum('Scheduled','Confirmed','Cancelled') DEFAULT 'Scheduled' COMMENT '看房状态，默认为已预约',
...
-- Dump completed on 2024-09-30 16:26:22
